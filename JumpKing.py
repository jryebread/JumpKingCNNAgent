...
class DDQN(object):
	def __init__(
		...
		self.optimizer = torch.optim.Adam(self.eval_net.parameters(), lr=0.001) #Why learning rate so large
	...
	def train(self, s0, a0, r, s1, sign):
		if sign == 1:
			if self.episode_counter % 2 == 0: #update target model every 2 episodes
				self.target_net.load_state_dict(self.eval_net.state_dict())
			self.episode_counter += 1

		self.memory_store(s0, a0, r, s1, sign)
		self.epsilon = np.clip(self.epsilon * self.epsilon_decay, a_min=0.01, a_max=None)
		#TODO: I CHANGED THIS VALUE TO 0.1
		# select batch sample
		if self.memory_counter > self.memory_size: #THIS IS SUSSY BAKA, WTF IS A MEMORY COUNTER
		...
		#we gather on 1st dimension to only get the q-values for the corresponding action we took in this batch
